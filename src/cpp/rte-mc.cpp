...
struct Photon
{
    ...
    double weight;
    Photon() : pos_n(3), pos(), dir(3), weight(1.0)
    {
        dir = eigen_vector_type<3>{0, 0, -1};
    }
...
inline Feel::po::options_description makeOptions()
{
    ...
    options.add_options()("diameter,d", po::value<double>()->default_value(0.1),
        ...
        "refractive index")("seed", po::value<int>()->default_value(42),
        ...
        "output-name,o", po::value<std::string>()->default_value("absorption"),
...
int main(int argc, char **argv)
{
    ...
    //----------------------------------------
    auto bvh =
        boundingVolumeHierarchy(_range = elements(mesh), _kind = "third-party");
    ...
    auto sample_disk = [&](double cx, double cy, double cz) {
        ...
        double th = 2 * M_PI * v;
        return eigen_vector_type<3>{cx + r * std::cos(th),
                                    cy + r * std::sin(th), cz};
    };
    ...
    for (int pid = 0; pid < Np; ++pid)
    {
        ...
        while (ph.weight > 0)
        {
            ...
            using BContext = typename std::remove_reference<
                decltype(*bvh)>::type::IntersectContext;
            auto hits = bvh->intersect(_ray = ray, _context = BContext::all,
                                       _parallel = false);
            struct Cand
            {
                double t;
            ...
            for (auto const &h : hits)
            {
                ...
                auto inter =
                    RaySimplexIntersection<3>::intersect(ph.pos, ph.dir, verts);
                if (inter.t > 0 && inter.t < s)
                    cands.push_back({inter.t, ce, inter.faceId});
                // debug
                // if (inter.faceId < 0 || inter.faceId > 3)
                // {
                //     std::cerr << "Invalid faceId from intersection\n";
                //     continue;
                // }
            }
            ...
            // std::cout << fmt::format(
            //     " Found {} candidates for interface hits.\n", cands.size());
            ...
            for (int i = 0; i < (int)cands.size(); ++i)
            {
                ...
                if (neigh == invalid_v<mesh_type::index_type>)
                    continue;
            ...
            // std::cout << fmt::format(
            //     " Depositing on {} segments before interface\n", end_idx);
            for (int i = 0; i < end_idx; ++i)
            {
                auto const &c = cands[i];
                double ds = c.t - last_t;
                auto id = c.elt;
                double ma_i = mu_a.localToGlobal(id, 0, 0),
                       ms_i = mu_s.localToGlobal(id, 0, 0), mt_i = ma_i + ms_i;
                double Ai = remW * (ma_i / mt_i) * (1.0 - std::exp(-mt_i * ds));
                absorption.plus_assign(id, 0, 0, Ai);
                remW *= std::exp(-mt_i * ds);
                last_t = c.t;
                // std::cout << fmt::format(
                //     "Deposited {:.6f} on element {}, remW->{:.6f}\n", Ai, id,
                //     remW);
            }
            ...
            {
                // std::cout << " No interface candidates, full-step
                // deposit.\n";
            ...
            if (iface_idx >= 0)
            {
                ...
                Eigen::Vector3d v0 = verts.col(fv[0]), v1 = verts.col(fv[1]),
                                v2 = verts.col(fv[2]);
                ...
                double R0 =
                    std::pow((n_old - n.localToGlobal(nextElt, 0, 0)) /
                                 (n_old + n.localToGlobal(nextElt, 0, 0)),
                             2);
            ...
            // std::cout << fmt::format(
            //     " Continuing beyond last hit at t={:.6f}, tail={:.6f}\n",
            //     lastHit, tail);
            if (tail > 1e-12 && remW > 0)
            {
                ...
                if (found)
                {
                    double ma1 = mu_a.localToGlobal(elt, 0, 0),
                           ms1 = mu_s.localToGlobal(elt, 0, 0), mt1 = ma1 + ms1;
                    double A1 =
                        remW * (ma1 / mt1) * (1.0 - std::exp(-mt1 * tail));
                ...
                if (!found)
                {
                    std::cerr << "Warning: photon lost, tail=" << tail << "\n";
                    break; // or return;
                }
            }
            ph.weight = remW;
    ...
    norm.save(_path = "save", _name = soption(_name = "output-name") + "-norm");
    // Save field data for dof usage in parallelization
    V0->save("save/" + soption(_name = "output-name") + "-fields");
